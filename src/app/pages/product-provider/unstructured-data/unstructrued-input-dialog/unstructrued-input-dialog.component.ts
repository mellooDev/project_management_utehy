...
@Component({
  selector: 'app-unstructrued-input-dialog',
  templateUrl: './unstructrued-input-dialog.component.html',
  styleUrl: './unstructrued-input-dialog.component.scss'
})
export class UnstructruedInputDialogComponent {
  packageName: string = '';
...
cycles: { 
  ...
  constructor(
    private dialogRef: MatDialogRef<UnstructruedInputDialogComponent>,
  ...
  ngOnInit(): void {
    ...
    if (this.data?.id) {
      ...
      let listPayWithNoDuration = this.listPay.filter(lp => lp.planType !== 'DURATION');
      this.packageTypesLoc = this.packageTypes.filter(pt => !listPayWithNoDuration.some(lp => lp.planType === pt.value));
    ...
    if (this.listPay.length > 0) {
      ...
      if (this.postpaidPeriodsLoc.length === 0) {
        ...
        this.packageTypesLoc = this.packageTypesLoc.filter(pt => pt.value !== 'DURATION');
      }
    }
...
addCycle(): void {
  if (this.cycles.length < this.postpaidPeriods.length) {
    this.cycles.push({ period: '', price: '', attributes: [{ attrName: '', attrDescription: '' }] });
  }
}
...
removeCycle(index: number): void {
  ...
  addAttribute(cycleIndex: number): void {
    this.cycles[cycleIndex].attributes.push({ attrName: '', attrDescription: '' });
  ...
  loadPackageDetail(data: any) {
    ...
    if(this.selectPackageType=='REQUEST'){
      this.goiCuocList= data.planOptions.map((option: { requestNumber: any; optionPrice: any; }) => ({
        requestNumber: option.requestNumber,
        optionPrice: option.optionPrice
      }));
    }
    if(this.selectPackageType=='DURATION'){
      this.cycles= data.planOptions.map((option: { optionDuration: any; optionPrice: any; planAttributes: any;}) => ({
  ...
  onSaveModal() {
    ...
    if(this.selectPackageType=='FULLPURCHASE' || this.selectPackageType=='PAYPERUSE'){
      const dataAPI = {
        "id": this.data?.id || new Date().getTime(), // Nếu có id thì giữ nguyên, nếu không thì để null (thêm mới)
        "planName":this.packageName,
        "productId": 0,
      ...
      var durationPlansInput = {
        ...
        .filter(({ requestNumber, optionPrice }) => 
          requestNumber != null && optionPrice != null && 
          !isNaN(optionPrice) && optionPrice > 0 && 
          !isNaN(requestNumber) && requestNumber > 0
        ).map(({ planName, optionPrice, optionDuration, description, planAttributes, requestNumber }) => ({
            optionName: planName,
            optionPrice,
            requestNumber,
            optionDuration,
    ...
    }else if(this.selectPackageType=='DURATION'){
      var durationPlansInput2 = {
        "id": this.data?.id || new Date().getTime(), // Nếu có id thì giữ nguyên, nếu không thì để null (thêm mới)
        "planName":this.packageName,
        "productId": 0,
        "planType":  this.selectPackageType,
        ...
        "status": "string",
        "duration": "string",
        planOptions: this.cycles.map(
          ({ price, period, attributes}) => ({
            "optionName": "string",
            "optionPrice": price,
            "optionDuration": period,
            "description": "string",
            "requestNumber": 0,
      ...
      this.paymentDataLocalRef.push(durationPlansInput2)
    }
